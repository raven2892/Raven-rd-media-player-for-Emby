...
Raven rd media player for Emby - 一款第三方原生的 Windows 媒体播放器和 Emby 客户端。
...
Raven 的目标是把 Emby 媒体库管理、远程播放、本地播放和 Windows 原生影音能力集中在一个统一的桌面应用中。

与只提供网页封装的客户端相比，Raven 直接使用 Windows 原生窗口、D3D11 视频输出、WASAPI 音频和 Windows 显示设备信息，可以更直接地处理 HDR、音频输出、字幕、播放控制和本地媒体文件。

Raven 由个人独立开发、测试和维护，将始终免费提供。所有核心功能不会因为是否赞助而受到限制。
